# Hi, I'm Jeziel 👋

I build scalable, resilient systems with an observability-first mindset — distributed infra, TypeScript, Rust, and real business impact.

<a href="https://github.com/jeziellopes"><img src="https://komarev.com/ghpvc/?username=jeziellopes&style=flat-square&color=grey"></a>

<!-- PROJECTS_START -->
### What I've shipped lately

- **[livemark](https://github.com/jeziellopes/livemark)** — Keep your GitHub profile README alive — self-updating zones powered by the GitHub API. Zero deps, written in Go.
- **[cv](https://github.com/jeziellopes/cv)** ⭐ 1 — Open-CV is a free, open-source resume generator. Edit cv.json, run one command, get a pixel-perfect HTML file and a print-ready PDF — no third-party login, no subscription, no watermark.
- **[vscdb-fix](https://github.com/jeziellopes/vscdb-fix)** ⭐ 32 — Repair tool for VS Code Copilot Chat sessions that disappear due to corrupted session index in state.vscdb.
- **[o11y-lab](https://github.com/jeziellopes/o11y-lab)** — A portfolio project demonstrating the three pillars of observability — traces, metrics, and logs — across a distributed microservices system on AWS.
<!-- PROJECTS_END -->

<!-- OSS_START -->
### Recent OSS

_No recent external contributions found._
<!-- OSS_END -->

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jezielcarvalho/)
[![jeziellopes.dev](https://img.shields.io/badge/jeziellopes.dev-000?style=flat-square&logo=vercel&logoColor=white)](https://jeziellopes.dev)
